...
describe('Determine Temperature Range', () => {
  ...
  it('Returns "unknown" for temperatures outside defined ranges', async () => {
    assert.strictEqual(await determineTempRange(null), baseString('unknown'));
    assert.strictEqual(await determineTempRange(undefined), baseString('unknown'));
    assert.strictEqual(await determineTempRange('string'), baseString('unknown'));
  });
});
